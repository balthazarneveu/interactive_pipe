...
class Control():
    ...
    @classmethod
    def from_tuple(cls, short_params, param_name :str =None):
        '''
        (0., [-1, 1.], name),  (0., [-1, 1., 1], name)...
        (True, name)
        '''
        if isinstance(short_params, bool):
            return cls(short_params, name=param_name)
        ...
        if name is None:
            name=param_name
        return cls(value_default, value_range=value_range, name=name, step=step)
