...
def test_headless_pipeline_multi_input(routing_indexes):
    ...
    with pytest.raises(AssertionError):
        ...
        pip = HeadlessPipeline(filters=[filt1, filt2], inputs=None, outputs=[BLEND_OUT, MAD_OUT], cache=True)
        out = pip.run()
    ...
    pip = HeadlessPipeline(filters=[filt1, filt2], inputs=[IMG_IN_1, IMG_IN_2], outputs=[BLEND_OUT, MAD_OUT], cache=True)
...
def test_headless_pipeline_no_input():
    ...
    filt2 = FilterCore(apply_fn=mad, name="mad", inputs=[FLAT], outputs=[MAD_OUT])
    filt3 = FilterCore(apply_fn=blend, inputs=[FLAT, MAD_OUT], outputs=[BLEND_OUT])

    pip = HeadlessPipeline(filters=[filt1, filt2, filt3], inputs=None, outputs=[FLAT, BLEND_OUT, MAD_OUT], cache=True)
    pip()

    pip = HeadlessPipeline(filters=[filt1, filt2, filt3], inputs=[], outputs=[FLAT, BLEND_OUT, MAD_OUT], cache=True)
    pip()
