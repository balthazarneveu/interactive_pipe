...
def test_headless_pipeline(tmp_path_factory):
    ...
    pip = HeadlessPipeline(filters=[filt1, filt2], inputs=[0], context={"ratio": 5})
...
def test_headless_pipeline_single_input(routing_indexes):
    ...
    pip = HeadlessPipeline(filters=[filt1, filt2], inputs=[IMG_IN], context={"ratio": 5}, cache=True)
    pip.inputs = input_image
    pip.run()
...
def test_headless_pipeline_no_input():
    ...
    pip = HeadlessPipeline(
        ...
        inputs=None,
        outputs=[FLAT, BLEND_OUT, MAD_OUT],
        cache=True,
    )
    pip()
