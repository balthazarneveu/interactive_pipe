...
Interactive Pipe allows you to define image processing filters and combine them into pipelines. This guide will walk you through the concepts from basic functions to interactive pipelines.

## Step 1: Defining Basic Functions

In this first part, we'll define very simple image processing filter functions. These will be unrelated to interactivity or sliders or anything fancy here!
...
> If we want to change the parameters manually, we'll have to provide each keyword parameter to each function and the code will be much more difficult to read unfortunately. Let's simply not do that, jump to the next section to see how interactive pipe will help you.
...
You can also export and load parameter tunings:

```python
sample_pipeline.export_tuning("my_tuning.yaml")
sample_pipeline.load_tuning("my_tuning.yaml")
```
...
# Please note that image_filters.py was kept the same since the beginning 
# and does not depend on interactive_pipe
interactive(
...
We'll decorate each image filter by providing some Controls which will allow creating a graphical interface: `@interactive(param_1=Control(...), param_2=Control(...), ...)`
